...
export const Indicators: React.FC<IndicatorsProps> = ({ state }) => {
  ...
  const chartData = useMemo(() => {
    ...
    return Object.entries(monthlyData).map(([name, km]) => ({ name, km })).sort((a, b) => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white p-6 sm:p-10 rounded-[2.5rem] border border-blue-50 shadow-sm">
        <div className="flex justify-between items-center mb-10"><h3 className="text-lg font-black text-blue-950 uppercase italic tracking-tighter leading-none">KM Total por Mês</h3><div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-blue-500"></div><span className="text-[9px] font-black uppercase text-gray-400">Distância Percorrida</span></div></div>
        <div className="h-72 sm:h-96 w-full -ml-6 sm:ml-0"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" /><XAxis dataKey="name" tick={{fontSize: 10, fontWeight: 900, fill: '#cbd5e1'}} axisLine={false} tickLine={false} /><YAxis tick={{fontSize: 10, fontWeight: 700, fill: '#cbd5e1'}} axisLine={false} tickLine={false} /><Tooltip contentStyle={{borderRadius: '20px', border: 'none', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', padding: '15px'}} cursor={{fill: '#f8fafc'}} formatter={(value) => [`${value} km`, 'Kilometragem']} /><Bar dataKey="km" fill="#2563eb" radius={[10, 10, 0, 0]} barSize={40}>{chartData.map((entry, index) => <Cell key={`cell-${index}`} fill={index === chartData.length - 1 ? '#1e1b4b' : '#2563eb'} />)}</Bar></BarChart></ResponsiveContainer></div>
      </div>
    </div>
  );
};
